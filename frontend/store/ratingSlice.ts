...
export const addRating = createAsyncThunk<Rating, {
    ...
    async (ratingData) => {
        const response = await axios.post(
            `http://192.168.29.152:8082/api/v1/ratings/${ratingData.userId}/products/${ratingData.productId}`,
            {
                rating: {
...
export const fetchRatingsByProduct = createAsyncThunk<Rating[], string>(
    ...
    async (productId, { getState }) => {
        ...
        const response = await axios.get(`http://192.168.29.152:8082/api/v1/ratings/products/${productId}`);
        return response.data;
    }
);
...
/* export const fetchRatingsByUser = createAsyncThunk<Rating[], string>(
    ...
    async (userId, { getState }) => {
        ...
        const response = await axios.get(`http://192.168.29.152:8082/api/v1/ratings/users/${userId}`);
        return response.data;
    }
);
 */
export const fetchRatingsByUser = createAsyncThunk<Rating[], string, { state: RootState }>(
    ...
    async (userId, { dispatch }) => {
      const response = await axios.get<Rating[]>(`http://192.168.29.152:8082/api/v1/ratings/users/${userId}`);
...
export const updateRating = createAsyncThunk<Rating, { ratingId: string; rating: Rating }>(
    ...
    async ({ ratingId, rating }) => {
        const response = await axios.put(`http://192.168.29.152:8082/api/v1/ratings/${ratingId}`, rating);
        return response.data;
    }
);
    ...
    async ({ userId, productId }) => {
        try {
            const response = await axios.get(
                `http://192.168.29.152:8082/api/v1/ratings/users/${userId}/products/${productId}`
            );
            return response.data;
        } catch (error) {
            return null;
...
>("rating/fetchAverageRating", async (productId, { getState }) => {
    ...
    const response = await axios.get(
        `http://192.168.29.152:8082/api/v1/ratings/products/${productId}/average`
    );
    return response.data;
});
...
export const removeRating = createAsyncThunk<void, string>(
    'rating/removeRating',
    async (ratingId) => {
        await axios.delete(`http://192.168.29.152:8082/api/v1/ratings/${ratingId}`);
    }
);
