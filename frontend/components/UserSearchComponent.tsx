  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (searchTerm.includes("@")) {
        response = await axios.get<string>(`http://192.168.29.152:8081/api/v1/users/email/${searchTerm}`);
      } else {
        response = await axios.get<string>(`http://192.168.29.152:8081/api/v1/users/username/${searchTerm}`);
      }
