...
/**
    ...
    public Inventory addInventory(String productId, String variantSku, String color,Map<String,Integer> sizeStock) {
        ...
        if (existingInventory.isPresent()) {
            ...
            newVariant.setColor(color); // Assuming "Default" color for simplicity, this can be customized
...
/**
    ...
    */
/**
     * Update the inventory, calculating total stock and available stock, and saving the updated inventory.
     *
     * @param inventory The inventory object to update.
     * @param sizeStock The updated size stock list.
     * @return The updated inventory.
     *//*

    private Inventory updateInventory(Inventory inventory, List<SizeStock> sizeStock) {
        // Recalculate the total stock and available stock
        int newStockQuantity = sizeStock.stream().mapToInt(SizeStock::getStockQuantity).sum();
        inventory.setStockQuantity(newStockQuantity);
        inventory.setAvailableStock(newStockQuantity - inventory.getReservedStock());
        inventory.setUpdatedAt(LocalDateTime.now());
